...
export type HomeMapDeepLink =
  | { mapEntryId: string; type: 'place' | 'portal' }
  | { space: SpaceSummary; type: 'space' };

export default function HomeMapApp({ initialContent }: { initialContent?: HomeMapDeepLink }) {
  ...
  useEffect(() => {
    if (!isUnlocked || !initialContent) return;
    const key = initialContent.type === 'space'
      ? `space:${initialContent.space.slug}`
      : `${initialContent.type}:${initialContent.mapEntryId}`;
    if (openedDeepLinkRef.current === key) return;
    openedDeepLinkRef.current = key;

    if (initialContent.type === 'space') {
      openSpaceInfo(initialContent.space);
      return;
    }
    void openMapEntryInfoById(
      initialContent.mapEntryId,
      initialContent.type,
    ).catch(() => undefined);
  }, [
    initialContent,
