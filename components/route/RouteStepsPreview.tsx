...
function StatusCard({ title, description, tone = 'default' }: { title: string; description: string; tone?: 'default' | 'error' }) {
  return (
    <div className={`${themeColors.panel.inset} border ${tone === 'error' ? themeColors.statusNotification.error.border : themeColors.border.secondary} ${themeColors.util.roundedLg} p-4 ${themeColors.transition}`}>
