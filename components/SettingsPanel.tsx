...
import MinecraftLinkOverlay from '@/components/settings/MinecraftLinkOverlay';
...
const SettingsPanel: React.FC<SettingsPanelProps> = ({
  ...
  const [isMinecraftLinkOpen, setIsMinecraftLinkOpen] = useState(false);
  const panelRef = useRef<HTMLDivElement | null>(null);
  const [panelHeight, setPanelHeight] = useState(0);
  const { theme, changeTheme: handleThemeChange } = useTheme();
  const { data: session, status } = useSession();
  const minecraftLink = useMinecraftLink(Boolean(session?.user), isMinecraftLinkOpen);
  ...
  const openMinecraftLink = () => {
    setIsMinecraftLinkOpen(true);
    void minecraftLink.startRequest();
  };

  return (
    <>
      <div className={`fixed bottom-4 right-4 z-50 transition-all duration-500 ease-out ${isExpanded ? 'w-80' : ''}`}>
        <div
        ...
        >
          <div className="flex flex-col items-end gap-2">
            <IconButtonRound onClick={onOpenNetherMap} aria-label="Ouvrir la carte du Nether">
              <MapIcon className={`w-6 h-6 ${themeColors.text.secondary}`} />
            </IconButtonRound>
        ...
        {!isExpanded ? (
          ...
          <div ref={panelRef}>
            <Panel>
              <SettingsHeader onClose={() => setIsExpanded(false)} />

              <div className={`p-4 ${themeColors.panel.primary} ${themeColors.blurSm} rounded-b-xl ${themeColors.transition}`}>
                <ThemeSelector value={theme} onChange={(nextTheme) => handleThemeChange(nextTheme)} showLabel={false} />
                ...
                <div className="space-y-2 -mb-1">
                  {status === 'loading' ? (
                    ...
                    <ProfileCard
                      ...
                      minecraftLinkStatus={minecraftLink.status}
                      onOpenMinecraftLink={openMinecraftLink}
  ...
  return (
    <div className={`p-4 border-b ${themeColors.border.primary} ${themeColors.panel.primary} ${themeColors.blurSm} ${themeColors.transition} rounded-t-xl`}>
      <div className="flex items-center justify-between">
        <h2 className={`text-xs font-semibold ${themeColors.text.secondary} ${themeColors.util.uppercase} ${themeColors.transition}`}>
          Paramètres
        </h2>
