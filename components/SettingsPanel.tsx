...
import AddPortalOverlay from './AddPortalOverlay';
...
const SettingsPanel: React.FC<SettingsPanelProps> = ({ onExpandedChange, onOpenMarket }) => {
    ...
    // Credentials modal removed; we use Discord OAuth
    // Market overlay moved to MarketButton component
    const { theme, changeTheme: handleThemeChange } = useTheme();
    const { data: session, status } = useSession();
    const [isAddPortalOpen, setIsAddPortalOpen] = useState(false);
    const [isAddPortalClosing, setIsAddPortalClosing] = useState(false);
    ...
    // Measure panel height when expanded to position BigTradeIcon above it with same gap
    useEffect(() => {
    ...
    const handleAddPortalClick = () => {
        if (status !== 'authenticated' || !session?.user) {
            signIn('discord');
            return;
        }
        setIsAddPortalClosing(false);
        setIsAddPortalOpen(true);
    };

    const handleCloseAddPortal = () => {
        setIsAddPortalClosing(true);
        if (portalTimeoutRef.current) {
            clearTimeout(portalTimeoutRef.current);
        }
    ...
    return (
        <div className={`fixed bottom-4 right-4 z-50 transition-all duration-500 ease-out ${
            ...
            {/* Global Market button: always above the settings trigger/panel */}
                ...
                <Panel>
                    ...
                    <div className={`p-4 ${themeColors.panel.primary} ${themeColors.blurSm} rounded-b-xl ${themeColors.transition}`}>
                        {/* Theme selector */}
                                ...
                                >
                                    {/* Discord icon (same as InfoOverlay link style) */}
                            ...
                            <ExternalLinks
                              onAddPortal={handleAddPortalClick}
                              disableAddPortal={status === 'loading'}
                            />
                        </div>
                    </div>
                </Panel>
            ...
            {/* No Auth Modal: OAuth redirect handled by NextAuth */}

            {/* Global Market Overlay */}
            {/* Global Market Overlay handled by MarketButton */}
            {isAddPortalOpen && (
                <Overlay isOpen={isAddPortalOpen} onClose={handleCloseAddPortal} closing={isAddPortalClosing}>
                    <AddPortalOverlay onClose={handleCloseAddPortal} closing={isAddPortalClosing} />
                </Overlay>
            )}
        </div>
    );
